...
def resample(df, sampleRate):
    ...
    """
    #date MM/DD/YYYY
    dates = list(df["Date(UTC-8)"])
    
    #time string HH:MM:SS
    times = list(df["Time(UTC-8)"])
...
#    print(valCounts)
    ...
    seenDates = []
    dates = list(pdArchivalData["Date(UTC-8)"])
    i = 0
    ...
    g = sns.catplot(data=df, kind="violin", x="Date(UTC-8)", y="Depth(m)", hue="Time of Day", split=True)
...
#        i += 1
    ...
    # convert to pandas dataframe
    d = {
        'Date': list(pdArchivalData["Date(UTC-8)"]), #[:i],
        'Depth(m)': list(pdArchivalData["Depth(m)"]), #[:i],
