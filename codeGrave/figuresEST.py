...
def resample(df, sampleRate):
    ...
    """
    #date MM/DD/YYYY
    dates = list(df["Date(EST)"])
    
    #time string HH:MM:SS
    times = list(df["Time(EST)"])
    ...
    # convert to pandas dataframe
    d = {
        'Date(EST)': resampledDates,
        'Time(EST)': resampledTimes,
...
def plotDepthTimeScatter(pdArchivalData, Id):
    ...
    g.fig.suptitle('Depth vs. Time(EST) for ' + Id)
...
def plotDepthTimeViolin(pdArchivalData, numDays, Id):
    ...
    for i in range(len(hours)):
        hour = hours[i]
        # call api later to get sunset and sunrise but
        # for now say 6pm - 6am is night -- -6 for EST
...
#    print(valCounts)
    ...
    seenDates = []
    dates = list(pdArchivalData["Date(EST)"])
    i = 0
    ...
    g = sns.catplot(data=df, kind="violin", x="Date(EST)", y="Depth(m)", hue="Time of Day", split=True)
...
#        i += 1
    ...
    # convert to pandas dataframe
    d = {
        'Date': list(pdArchivalData["Date(EST)"]), #[:i],
        'Depth(m)': list(pdArchivalData["Depth(m)"]), #[:i],
        'Hour': list(pdArchivalData["Hour"]),
        'Time of Day (EST)': isDay #[:i]
        }
    df = pd.DataFrame(d)
    ...
    g = sns.catplot(data=df, kind="violin", x="Hour", y="Depth(m)", hue="Time of Day (EST)", split=False)
